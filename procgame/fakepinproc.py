...
class FakePinPROCPlayback(FakePinPROC):
    ...
    _start_time = 0
    """ The simulator start time so we know how to calculate simulator time """

    _playback_file = None
    """ Playback file object that we read from """

    _events = dict()  #
    """ We store events in a dictionary keyed by their simulator time """

    _event_timestamps = None
    """ Event timestamps are stored in a list so they can be sorted so we access the dictionary in order."""

    _states = [0] * 256  # Local switch state repository

    def __init__(self, machine_type, playback_file="playback.txt"):
        super(FakePinPROCPlayback, self).__init__(machine_type)

        if os.path.exists(playback_file):
            self._playback_file = open(playback_file, 'r')  # Open our playback file for reading
