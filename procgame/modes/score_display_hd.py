...
from procgame.game import Mode, advancedmode
import yaml
...
class ScoreDisplayHD(advancedmode.AdvancedMode):
    ...
    def __init__(self, game, priority, yaml_file_path="config/new_score_display.yaml"):
        ...
        # Create and add a single player background from the Animations Name.
        sp_name = value_for_key(values, 'ScoreLayout.SinglePlayer.Background.Animation.Name', None)
        from_saved = value_for_key(values, 'ScoreLayout.SinglePlayer.Background.Animation.from_saved', False)
        self.single_player["bg"] = self.get_background_animation(sp_name, from_saved)
        self.single_player_layers.append(self.single_player["bg"])

        # Single player Credit layer
        self.single_player["credit_layer"] = self.game.dmdHelper.generateTextLayerFromYaml(
            value_for_key(values, 'ScoreLayout.SinglePlayer.CreditIndicator'))
        self.single_player_credit_template = value_for_key(values, 'ScoreLayout.SinglePlayer.CreditIndicator.format',
                                                           "FREE PLAY")
        ...
        self.single_player["ball_number"] = self.game.dmdHelper.generateTextLayerFromYaml(
            value_for_key(values, 'ScoreLayout.SinglePlayer.BallNumber'))
        self.single_player_ball_number_template = value_for_key(values, 'ScoreLayout.SinglePlayer.BallNumber.format',
                                                                "BALL _")

        if ("_" in self.single_player_ball_number_template):
            self.single_player["ball_number"].set_text(
                self.single_player_ball_number_template.replace("_", "%d" % self.game.ball))
        ...
        if (mp_name == 'Same' or mp_name == sp_name or mp_name == None):
            self.multiplayer["bg"] = self.single_player["bg"]
        else:
            self.multiplayer["bg"] = self.get_background_animation(mp_name, from_saved)
        ...
        if (credit_indicator_data is None):
            ...
            self.multiplayer_credit_template = value_for_key(values, 'ScoreLayout.MultiPlayer.CreditIndicator.format',
                                                             "FREE PLAY")
        ...
        if (ball_num_data is None):
            ...
            self.multiplayer_ball_number_template = value_for_key(values, 'ScoreLayout.MultiPlayer.BallNumber.format',
                                                                  "BALL _")
            if ("_" in self.multiplayer_ball_number_template):
                self.multiplayer["ball_number"].set_text(
                    self.multiplayer_ball_number_template.replace("_", "%d" % self.game.ball))
            else:
        ...
        for idx, playerName in enumerate(["PlayerOne", "PlayerTwo", "PlayerThree", "PlayerFour"]):
            ...
            self.multiplayer["p%d" % idx] = self.game.dmdHelper.generateTextLayerFromYaml(
    ...
    def set_background(self, new_key, from_generated=False):
        """ Sets the high score background. From generated True gets from the games saved generated_sequences """

        if len(self.game.players) <= 1:
            self.single_player_layers[0] = self.get_background_animation(new_key, from_generated)
            self.update_layer_1p()
        else:
            self.multiplayer_layers[0] = self.get_background_animation(new_key, from_generated)
            self.update_layer_4p()

    def get_background_animation(self, key, from_generated=False):
        """ Gets an animation from the game dictionary. If from_generated is True it will retrieve a layer from the generated_sequences."""
        if from_generated:
            try:
                return self.get_sequence(key).layer
            except:
                return self.game.animations[key]
        else:
            return self.game.animations[key]
